...
- **PropertySidebar**: Tabs (Properties | Scripts/Code | Assets | Presets). When **Properties** is active it renders **PropertyPanel**. The tab row’s **right side** shows **Dock / float** (layout toggle, persisted) and **Restore saved position and rotation** (reset icon) while the handler is wired: Builder calls `SceneView.updateEntityPose` per **unlocked** selected entity with that entity’s persisted world `position` / `rotation` (defaults from [`world.ts`](../src/types/world.ts)); **scale** is unchanged. The reset control is disabled when nothing is selected or every resolved entity is locked. Inverse direction from **Refresh from physics** (scene → document).
...
- **EntitySidebar** (left, **Camera** tab when control is Follow): **Avatars** chips pick the follow-camera target; **Edit** (`aria-label` “Edit avatar settings”) opens **AvatarDialog**, which includes the same chip row ([`AvatarDialog`](../src/components/AvatarDialog.tsx) + [`EntitySidebar`](../src/components/EntitySidebar.tsx)) so you can switch the active avatar and which entity the dialog edits without returning to the sidebar. In **Playable & camera defaults**, **Set current camera as default** captures the live follow/orbit state from [`SceneView`](../src/components/SceneView.tsx) (`getAvatarFocusSnapshot`) and persists `orbitYaw`, `orbitPitch`, `orbitDistance`, `mode`, `control`, `distance`, `height`, and FOV under `entity.avatar.preferredCamera` (JSON schema). [`AvatarSession`](../src/runtime/avatarSession.ts) loads those values when focusing an avatar that has no in-memory session snapshot yet. When the Builder **Target** (follow) changes, `SceneView` calls `session.syncWorld(world)` then `setCurrentAvatar(target)` so each avatar’s saved angle and mode are applied instead of reusing the previous target’s orbit. The dialog’s **Transformers** and **Scripts** sections show thin name-lists (IDs/labels). Clicking an item opens the **Workspace overlay** anchored to that item. Direct **"Open Workspace — Transformers"** / **"Open Workspace — Scripts"** buttons are also available. Full editing (Monaco, event controls, Organize) happens in Workspace. `EntityScriptEditor` and `ScriptDialog` were removed in Workspace Phase 8 (2026-05-09).
...
Polling (or any programmatic display update) must **not** call `onWorldChange` or `updateEntity`. Live display uses a separate state **livePoses** (passed into `PropertySidebar`, sourced from [`LivePosesPoll.tsx`](../src/components/LivePosesPoll.tsx) so `Builder` does not re-render on each poll). Updating that state only re-renders the inspector subtree; it does not change `world`, so SceneView’s effect (which depends on `world`) does not run and the scene is not reloaded. User edits continue to go through `onWorldChange` / `onEntityPoseChange` only.
...
```
src/
├── pages/Builder.tsx           # SceneView ref, getCurrentPose, handleRefreshFromPhysics, handleCloneEntity; wires `LivePosesPoll` → PropertySidebar
├── components/LivePosesPoll.tsx # Interval → `getAllPoses`; render-prop updates inspector subtree only
├── components/
