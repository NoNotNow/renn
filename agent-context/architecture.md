...
5. Entity list, **PropertyPanel** (multi-select aware: merged fields, bulk apply), **ScriptPanel** (script intersection + attach/detach on all selected), **AssetPanel** read/write via `updateWorld()` and `updateAssets()` from ProjectContext. Changes trigger SceneView to re-run its effect and rebuild the scene when needed.
