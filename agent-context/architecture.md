...
5. **Edit-Modus (Navigation)** in the Builder: toggle with **Ctrl+E** or **View → Edit-Modus** (checkmark when active). Red record-style dot top-right on the canvas. While on: **WASD free-fly** via `CameraController.setForceFreeFlyNavigation` without persisting world/camera config; **no** `executeTransformers`; **no** Rapier `step` (simulation frozen); **no** script `runOnUpdate` or collision hooks; debug forces are not applied during the pause. Gizmo and picking stay available. Nothing is written to the world document for this mode.
