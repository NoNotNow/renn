...
- **`api.raycast(origin, fwd, maxDistance?, options?)`**: Cast a ray from a raw `Vec3` origin (typically `input.position`) in direction `fwd`. No entity is automatically excluded — offset the origin or filter `result.entityId` in code if needed. Returns `{ hit: boolean, distance: number, entityId: string }`. `maxDistance` defaults to 100. Returns no-hit when physics is unavailable or direction is zero-length. Optional `options`: `{ visualize?: boolean, hitColor?: string, missColor?: string }` — when `visualize: true` (builder visualize mode only), draws a debug line to the hit point (`hitColor`, default `red`) or to `maxDistance` on miss (`missColor`, default `green`). Example: `const r = api.raycast(input.position, api.getForwardVector(input.rotation), 15, { visualize: true }); if (r.hit) { return { force: api.scaleVec3(fwd, -200) } }`.
