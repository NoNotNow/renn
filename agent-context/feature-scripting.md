...
- **Code sidebar** (`CodingTabPanel`): Right sidebar **Scripts**, **Transformers**, and **Code** segments. **Scripts** is entity event scripts (**Hide toolbar** / **Show toolbar** for more Monaco space). **Transformers** is the full stack editor. **Code** is the named custom-transformer Monaco workspace (debounced live apply, params JSON, enabled LED).
