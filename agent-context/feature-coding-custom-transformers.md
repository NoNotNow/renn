...
- **Transformer code** segment via [`CustomTransformerCodeTab.tsx`](../src/components/CustomTransformerCodeTab.tsx):
  ...
  - **Pop out**: **Pop out** beside the Code label opens Monaco plus compile/runtime error panels in **`createPortal`**. The portal **container** is **`document.fullscreenElement` when native fullscreen is active** (otherwise the overlay would not paint) and **`document.body` otherwise** so Monaco stays aligned with auxiliary DOM such as **`monaco-area-container`** (overflow widgets appended under `document.body`; nesting the portal only inside the Builder column desynchronizes sizing and can collapse to ~1×1). Reactive on fullscreenchange. **Pop out** also **collapses left and right drawers** (same closed state as entering fullscreen); toggles keep working afterward. **Important:** [`Sidebar.tsx`](../src/components/layout/Sidebar.tsx) must **not unmount** drawer children when `isOpen` is false (children stay mounted but `display: none` when closed) — otherwise **`collapseSideDrawers` would destroy `CustomTransformerCodeTab` state** and the portal would never mount. Softer full-bleed dim ([`theme.bg.modalBackdropSoft`](../src/config/theme.ts)), large centered **frosted-glass** shell (`theme.bg.modalGlass` + [`theme.effects.modalGlassBlur`](../src/config/theme.ts); `min(95vw, 1600px)` × `min(92vh, 1200px)`), **Escape** / backdrop click / header **×** / **Dock editor** flushes pending code (same as tab switch) and closes; inline editor is replaced by a short placeholder until docked.
