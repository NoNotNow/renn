...
- **`api.watch`**: publishes a labeled value to the Workspace **Watch** panel when Builder has Workspace open with a single selected entity (`transformerWatchBridge`). `api.watch(label, value)` or single-arg `api.watch(value)` (label defaults to `value`). Updates only when `watch()` is called; stale labels from prior simulation runs remain until **Clear**. No-op when the bridge is disabled. The Watch panel uses shared [`WorkspaceFloatingDrawer`](../src/components/workspace/WorkspaceFloatingDrawer.tsx) (draggable, resizable on all edges/corners); see [`feature-ui-infrastructure.md`](feature-ui-infrastructure.md).
...
| `watch` | `(label, value) → void` or `(value) → void` | Builder Workspace only: show `label: value` in the draggable Watch panel (top-right of Monaco). One argument uses label `value`. Two arguments require non-empty string `label`; invalid args throw **`[TransformerRuntimeApi.watch]`**. Updates only on call; stale labels persist until **Clear**. No-op when bridge disabled (Workspace closed, multi-select, tests unless wired). |
