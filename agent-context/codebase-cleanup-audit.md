...
- **`src/hooks/useBuilderFullscreenChrome.ts`** — owns the left/right drawer open state (`useLocalStorageState`, persisted), the `usePointerRevealTimeout` reveal flag, the fullscreen API support flag, the saved-drawer snapshot (collapse on enter, restore on exit), and the `builderColumnRef`; **`collapseSideDrawers`** closes both drawers without fullscreen snapshot bookkeeping (e.g. Workspace). Returns the merged `builderChromeIdleHidden` derived flag and the `handleSceneFullscreenChange` callback used by `<SceneView onFullscreenChange={…}>`.
