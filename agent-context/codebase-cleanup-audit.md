...
- **`src/hooks/useBuilderFullscreenChrome.ts`** — owns the left/right drawer open state (`useLocalStorageState`, persisted), the `usePointerRevealTimeout` reveal flag, the fullscreen API support flag, the saved-drawer snapshot (collapse on enter, restore on exit), and the `builderColumnRef`; **`collapseSideDrawers`** closes both drawers without fullscreen snapshot bookkeeping (e.g. transformer code pop-out). Returns the merged `builderChromeIdleHidden` derived flag and the `handleSceneFullscreenChange` callback used by `<SceneView onFullscreenChange={…}>`.
- Pointer listeners are still only attached when `isFullscreenEnabled()` is true.
- **`src/hooks/useBuilderFullscreenChrome.test.ts`** — 7 tests: defaults, localStorage seeding, drawers collapse-on-enter / restore-on-exit, `collapseSideDrawers` + fullscreen round-trip, manually-collapsed drawer survives the round-trip, `builderChromeIdleHidden` flips with chrome reveal, `builderColumnRef` is stable across rerenders.
...
|------|-------|---------------------|
| `pages/Builder.tsx` | **1177** | ✅ Phase 15: `useTextureMakerSession` extracted (2009 → 1177, -41%). Remaining: selection/group state (`useExplorerSelection`), import/export handlers, pose-sync `syncPosesThen`. |
