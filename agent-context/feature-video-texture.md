...
- **Three.js:** [`createAssetResolverFromGetter`](../src/loader/assetResolverImpl.ts) exposes `isVideoAsset` + `loadVideoTexture`. [`loadWorld`](../src/loader/loadWorld.ts) accepts a **getter** `() => Map<string, Blob>` so [`SceneView`](../src/components/SceneView.tsx) keeps **one** resolver for the scene: meshes and `VideoTexture` `<video src>` stay on valid `blob:` URLs until effect teardown. [`materialFromRef`](../src/loader/createPrimitive.ts) uses `THREE.VideoTexture` (muted, loop, inline) for video maps. [`disposeMaterialOrArray`](../src/utils/videoTextureLifecycle.ts) pauses the underlying `<video>` before `dispose()`.
