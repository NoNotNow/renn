...
export interface EntityScriptEditorProps {
  world: RennWorld
  entityId: string
  onWorldChange: (world: RennWorld) => void
  /** Min height of the Monaco host (px). */
  editorMinHeight?: number
  /** When set, Monaco uses this fixed height (px); use in dialogs where flex chain has no height. */
...
export default function EntityScriptEditor({
  ...
  const dropdownOptions = [...attachedScriptIdsOrdered, ...scriptIds.filter((id) => !entityScriptIds.includes(id))]
  ...
  useEffect(() => {
    ...
    if (attachedScriptIdsOrdered.length > 0) {
  ...
  }, [entityId, attachedScriptIdsOrdered.join(','), scriptIds.join(','), selectedId])
  ...
  const handleApply = () => {
    ...
    pushUndo()
    uiLogger.change('EntityScriptEditor', 'Apply script', { scriptId: selectedId, contentLength: draftSource.length })
  ...
  const handleAdd = () => {
    ...
    pushUndo()
    uiLogger.click('EntityScriptEditor', 'Add new script', { scriptId: id })
    const nextScripts = { ...scripts, [id]: { event: 'onUpdate' as const, source: '// ctx.log(ctx.entity.id);' } }
    onWorldChange({
      ...world,
      scripts: nextScripts,
      entities: world.entities.map((e) =>
        e.id === entityId ? { ...e, scripts: [...(e.scripts ?? []), id] } : e,
      ),
    })
    setSelectedId(id)
  }
  ...
  const handleDetachFromEntity = () => {
    ...
    uiLogger.click('EntityScriptEditor', 'Detach script from entity', { scriptId: selectedId, entityId })
    const nextEntityScripts = entityScriptIds.filter((sid) => sid !== selectedId)
    onWorldChange({
      ...world,
      entities: world.entities.map((e) => (e.id === entityId ? { ...e, scripts: nextEntityScripts } : e)),
    })
  ...
  const isAttachedToEntity = Boolean(selectedId && entityScriptIds.includes(selectedId))
  ...
  const handleEntityScriptsChange = (nextScriptIds: string[]) => {
    onWorldChange({
      ...world,
      entities: world.entities.map((e) => (e.id === entityId ? { ...e, scripts: nextScriptIds } : e)),
    })
  }
  ...
  return (
    ...
    >
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
          ...
          >
            ...
            {attachedScriptIdsOrdered.length > 0 && (
              <optgroup label="Attached to this entity">
          ...
          <button
            ...
            disabled={!selectedId || !isAttachedToEntity}
            title={
              !isAttachedToEntity
                ? 'This script is not attached to this entity'
                : 'Remove this script from this entity (script stays in world)'
            }
            style={{
              padding: '6px 12px',
              borderRadius: 6,
      ...
      <div
        ...
        style={{
          flex: editorHeightPx != null ? undefined : 1,
          minHeight: editorHeightPx ?? editorMinHeight,
