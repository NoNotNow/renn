...
- [x] **Camera controller** – Follow / third-person / first-person; target entity, distance, height (`src/camera/cameraController.ts`).
...
- [x] **ZIP export/import** – Export: `world.json` + `assets/`; import with validation and optional replace.

### Builder UI
- [x] **Builder page** – Toolbar (New, Save, Save as, Download, Upload, project list, Delete, Play); entity list; camera target/mode; main canvas (SceneView).
- [x] **Property panel** – Edit selected entity (position, rotation, scale, bodyType, mass, restitution).
...
- [ ] **Entity creation/deletion in builder** – Add/remove entities from the world from the UI (not only edit existing).
- [ ] **Shape editing in property panel** – Edit shape type and params (box size, sphere radius, etc.) per entity.
